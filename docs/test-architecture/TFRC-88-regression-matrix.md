...
| 11 | 菜单展示环境/组织/账户，并支持账户切换和退出 | `GlobalManagerAccount` — `shows current Context and switches only through the store transaction action`、`offers one clear sign-out action without an ineffective re-login path`、`surfaces pending account selection from every page entry`；`managerStore` — `switches accounts through the backend transaction and accepts only its Context snapshot`、`logout commits the backend signed-out Context without mutating it locally` |
