...
| 13 | 过渡态阻止真正冲突的 Runtime/连接操作并显示当前动作；远端连接转换不阻塞本地 MCP 生命周期 | `src/test/components/ComputerRuntime.test.tsx` — `keeps local MCP lifecycle controls available during a remote connection transition`；`src-tauri/tests/smcp_connection_lifecycle_test.rs` — `remote_reconnect_does_not_block_local_mcp_lifecycle`；Lifecycle action matrix |
